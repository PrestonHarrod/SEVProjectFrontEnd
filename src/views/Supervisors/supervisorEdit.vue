...
export default {
  ...
  created() {
    ...
    UserServices.getUser(this.id)
      ...
      //grab each role that the supervisor has and loop through the checkboxes and check the ones in the system
  ...
  methods: {
    ...
    updateSupervisor() {
      ...
      //delete all the supervisor's userRoles with their ID = userID
      UserRoleServices.deleteUserRole(this.supervisor.userID)
        .then(() => {
          console.log(
            "called to delete all the userRoles for user: " +
              this.supervisor.userID
          );
        })
      ...
      UserOrgServices.deleteUserOrg(this.supervisor.userID)
        .then(() => {
          console.log(
            "called to delete all the userOrgs for user: " + this.supervisor.userID
          );
        })
      ...
      //finally, update the main components of the supervisor
      UserServices.updateUser(this.supervisor)
        ...
        .catch((error) => {
          console.log("There was an error:", error.response);
          alert("ERROR:Edit Supervisor unsuccessful.");
        });
    },
  },
};
